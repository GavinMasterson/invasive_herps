...
inv_family <- a_range_tbl %>% 
              group_by (Family) %>% 
              count() %>%           #creates a column with "n" for the counts
              rename(Tot_num_inv = n) %>% 
              arrange(desc(Tot_num_inv))
...
a <- data.frame(Family = c(inv_family$Family), Tot_num_inv = rep(0,18), frame = rep('a',18))
b <- data.frame(Family = c(inv_family$Family), Tot_num_inv = c(inv_family$Tot_num_inv), frame = rep('b',18))
invfa_anim_df <- rbind(a,b); rm(a,b)

invfam_anim <- ggplot(invfa_anim_df, aes(x = Family, y = Tot_num_inv, fill = Family)) +
