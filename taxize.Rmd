...
One of the very useful helper functions to `gnr_resolve` is `gnr_datasources`. Calling the function provides a tibble containing a very detailed breakdown of the datasources used to resolve the name/s supplied to `gnr_reolve`. In the code below I use `gnr_datasources` to see which datasources have been within the last 150 days. I used the call to `Sys.time` but for reproducibility I have inserted the actual system time in the code below.
